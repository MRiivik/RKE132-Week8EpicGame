...
//string[] heroes = {"Harry Potter", "John Wick", "Lara Croft", "Scooby-Doo"};
//string[] villains = { "Voldemort", "The Joker", "Darth Vader", "Dracula", "Sauron" };

string hero = GetRandomValueFromArray(heroes);
string heroWeapon = GetRandomValueFromArray(weapons);
int heroHP = GetCharacterHP(hero);
int heroStrikeStrength = heroHP;
Console.WriteLine($"Today {hero} ({heroHP} HP) using a {heroWeapon} saves the day!");
...
Console.WriteLine($"Today {villain} ({villainHP} HP) using a {villainWeapon} tries to take over the world!");
